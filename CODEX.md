...
- 4.x 系の公開パッケージは Svelte 5 専用とし、Svelte 4 互換を保証しない。
- 新規コンポーネントでは `$props()`、`$state()`、`$derived()`、`$effect()` などの runes とイベント属性を使用する。
- 既存コンポーネントの legacy 構文は Svelte 5 上で動作する間は許容し、関連する改修時に公開 API と Storybook テストを維持しながら段階的に移行する。
- 詳細な判断基準と検証手順は `docs/SVELTE_5_MIGRATION.md` に従う。

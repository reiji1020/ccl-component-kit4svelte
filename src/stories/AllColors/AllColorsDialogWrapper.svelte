<script lang="ts">
  ...
  let vividEntries: Entry[] = Object.entries(CCLVividColor).map(([name, color]) => ({ name, color, open: false }));
  ...
  <div class="color-grid">
    {#each vividEntries as item, i}
      <div class="color-sample">
        ...
        <Dialog open={item.open} title={item.name} borderColor={item.color} on:close={() => closeAt(i)}>
